...
from database.db import get_all_students, create_student, get_student_subjects, get_student_attendance, unenroll_student_to_subject
import time
...
def student_screen():
    ...
    if join_code:
        st.success(f"Join code detected: {join_code}")
    ...
    if show_registration:
        with st.container(border=True):
            st.markdown(
            "<h2 style='color:black;'>Register new Profile</h2>",
            unsafe_allow_html=True
        )
